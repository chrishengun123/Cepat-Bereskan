...
@onready var organic_bin = $organic_bin
@onready var inorganic_bin = $inorganic_bin
...
func _unhandled_input(event: InputEvent) -> void:
	if Input.is_action_just_pressed("click"):
		...
		for trash:Trash in unsorted:
			trash.held = false
			if is_overlapping(trash, organic_bin) and trash.type == "organic":
				unsorted.erase(trash)
				trash.queue_free()
			if is_overlapping(trash, inorganic_bin) and trash.type == "inorganic":
				unsorted.erase(trash)
				trash.queue_free()
...
func is_overlapping(trash: Sprite2D, bin: Sprite2D) -> bool:
